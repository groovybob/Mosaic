...
colours = []  #  average brighnesses of pics
tilecolours = []  #  average brightnesses of each tile
...
#  find the average colours of each image
for file in os.listdir(dir):  # open each file and find average brightness
    filename = os.fsdecode(file)
    if filename.endswith(".jpg"):
        name = dir+'/'+file
        colour = average_colour(name)
        colours = np.append(colours, colour)
...
for i in range(int(img.shape[1]/tile_size[0])):
    for j in range(int(img.shape[1]/tile_size[0])):
        ...
        tilecolour = average_colour("debug_" + str(i) + "_" + str(j) + ".jpg")
        tilecolours = np.append(tilecolours, tilecolour)
        os.remove("debug_" + str(i) + "_" + str(j) + ".jpg")

#  begin building mosaic
total = int((img.shape[1]/tile_size[0])**2)
#  brightness_factor = np.average(tilecolours)/np.average(colours)
#  tilebrights = tilecolours/brightness_factor
...
for k in range(total):
    ...
    selected_im = find_nearest(colours, tilecolours[k])
...
#  new_im.show()
new_im.save(dir+"/outcolour.jpg", "JPEG", quality=80, optimize=True, progressive=True)
